 ...
 * ideal/acceptable 2-3 hour windows. No browser geolocation required.
 */
...
const GPT_WEATHER_CONFIG = Object.freeze({
  ...
  windowHours: 3,
  // These remain configurable so the application's existing weather rules
  // can be supplied without changing this module.
  tempMin: 10,
  tempMax: 30,
  precipitationMin: 0,
  precipitationMax: 20
...
function gptWeatherCondition(code) {
  return GPT_WEATHER_CODE[Number(code)] || 'Unknown';
}

function gptWeatherRuleSource() {
  // Prefer the application's existing global rule names when available.
  const candidates = [
    window.WEATHER_RULES,
    window.weatherRules,
    window.WEATHER_CONFIG,
    window.weatherConfig
  ].filter(Boolean);
  return candidates[0] || GPT_WEATHER_CONFIG;
}
...
function gptDateOnly(value) {
  return new Date(`${value}T12:00:00`);
}

function gptDaysBetween(a,b) {
  return Math.round((gptDateOnly(b)-gptDateOnly(a))/86400000);
}

function gptReadLastWashDate() {
  const candidates = [
    window.GPT_STORE?.lastWashDate,
    window.GPT_STORE?.lastWash,
    window.GPT_STORE?.washHistory?.at?.(-1)?.date,
    window.gptWashStore?.lastWashDate,
    window.gptWashHistory?.at?.(-1)?.date
  ];
...
function gptTargetDate(lastWashDate) {
  if (!lastWashDate) return null;
  const d = gptDateOnly(lastWashDate);
  d.setDate(d.getDate()+GPT_WEATHER_CONFIG.targetWashDays);
...
function gptHourlyRecords(data) {
  ...
  return (h.time || []).map((time,i) => ({
    ...
    rainProbability: Number(h.precipitation_probability?.[i]),
    precipitation: Number(h.precipitation?.[i] || 0),
    code: Number(h.weather_code?.[i]),
    condition: gptWeatherCondition(h.weather_code?.[i]),
...
function gptScoreHour(hour, rules) {
  ...
  if (hour.code >= 95) score -= 80;
  else if ([65,67,75,82,86].includes(hour.code)) score -= 45;
  else if ([61,63,71,73,80,81,85].includes(hour.code)) score -= 20;
...
function gptFindWindows(records, targetDate, lastWashDate) {
  ...
  earliest.setDate(earliest.getDate()-GPT_WEATHER_CONFIG.searchBeforeTargetDays);
  const earliestDate = earliest.toISOString().slice(0,10);
  const eligible = records.filter(r => r.date >= earliestDate && (!lastWashDate || r.date > lastWashDate));
  ...
  for (let i=0;i<=scored.length-GPT_WEATHER_CONFIG.windowHours;i++) {
    ...
    const end = new Date(chunk[chunk.length-1].time); end.setHours(end.getHours()+1);
  ...
  return windows.sort((a,b)=>Number(b.ideal)-Number(a.ideal) || b.score-a.score || a.distanceToTarget-b.distanceToTarget);
}
...
function gptDailySummary(data,targetDate) {
  ...
  return (d.time||[]).map((date,i)=>({
    ...
    condition:gptWeatherCondition(d.weather_code?.[i]),
    max:Number(d.temperature_2m_max?.[i]),
    min:Number(d.temperature_2m_min?.[i]),
    rainProbability:Number(d.precipitation_probability_max?.[i]),
...
function gptWeatherRecommendation(data,lastWashDate=gptReadLastWashDate()) {
  const targetDate=gptTargetDate(lastWashDate);
  const hourly=gptHourlyRecords(data);
  const windows=gptFindWindows(hourly,targetDate,lastWashDate);
  const ideal=windows.find(w=>w.ideal)||null;
  const acceptable=windows[0]||null;
  return {
    location:{lat:GPT_WEATHER_CONFIG.lat,lon:GPT_WEATHER_CONFIG.lon},
    lastWashDate,
    targetDate,
    daysUntilTarget:targetDate?gptDaysBetween(new Date().toISOString().slice(0,10),targetDate):null,
    daily:gptDailySummary(data,targetDate),
    hourly,
    windows,
    idealWindow:ideal,
    bestAvailable:ideal||acceptable,
    recommendation:ideal ? 'ideal' : acceptable ? 'acceptable' : 'none',
    rules:gptWeatherRules(),
    current:data.current||null
  };
}

window.GPT_WEATHER = Object.freeze({
  config:GPT_WEATHER_CONFIG,
  fetch:gptFetchWeather,
  build:gptWeatherRecommendation,
  condition:gptWeatherCondition,
  targetDate:gptTargetDate
});
